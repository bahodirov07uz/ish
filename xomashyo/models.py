...
class Xomashyo(models.Model):
    ...
    yetkazib_beruvchi = models.ForeignKey(
        'YetkazibBeruvchi', on_delete=models.SET_NULL, null=True
    )
...
# ─────────────────────────────────────────────────────────────────
# XOMASHYO HARAKAT  (eski tuzilma — o'zgarmaydi)
# ─────────────────────────────────────────────────────────────────

class XomashyoHarakat(models.Model):
    """
    Eski tuzilma saqlanib qoldi.
    Endi to'g'ridan-to'g'ri yaratilmaydi — ChiqimItem.save() orqali avtomatik yaratiladi.
    chiqim_item reverse relation orqali qaysi item yaratganini bilsa bo'ladi.
    """
    HARAKAT_TURLARI = [
        ('kirim',            'Kirim'),
        ('chiqim',           'Chiqim'),
    ...
    def __str__(self):
        return f"{self.xomashyo.nomi if self.xomashyo else '—'} | miqdor: {self.miqdori}"
    ...
    def save(self, *args, **kwargs):
        """Eski logika o'zgarmadi — stokni yangilaydi."""
        self.clean()
